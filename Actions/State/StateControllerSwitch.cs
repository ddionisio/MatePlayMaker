using M8;

namespace HutongGames.PlayMaker.Actions.M8 {
    [ActionCategory("Mate State")]
    [Tooltip("Compare current state in StateController with multiple states.")]
    public class StateControllerSwitch : ComponentAction<StateController> {
        [RequiredField]
        [CheckForComponent(typeof(StateController))]
        public FsmOwnerDefault gameObject;

        [CompoundArray("State Switches", "Compare Int", "Send Event")]
        [ObjectType(typeof(State))]
        public FsmObject[] compareTo;
        public FsmEvent[] sendEvent;

        public FsmBool everyFrame;

        public override void Reset() {
            gameObject = null;
            compareTo = new FsmObject[1];
            sendEvent = new FsmEvent[1];
            everyFrame = false;
        }

        public override void OnEnter() {
            DoCheck();

            if(!everyFrame.Value)
                Finish();
        }

        public override void OnUpdate() {
            DoCheck();
        }

        void DoCheck() {
            var go = Fsm.GetOwnerDefaultTarget(gameObject);
            if(!UpdateCache(go))
                return;

            var curState = cachedComponent.state;

            for(int i = 0; i < compareTo.Length; i++) {
                if(curState == compareTo[i].Value) {
                    Fsm.Event(sendEvent[i]);
                    return;
                }
            }
        }
    }
}